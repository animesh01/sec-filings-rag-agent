"""LedgerIQ RAG pipeline — the retrieval-augmented core (shared by both sources).
...
The pipeline stages, each independently inspectable in the UI:
    ...
    1. chunk      — split documents into overlapping passages
...
REFUSAL = ("I could not find this in the retrieved documents, so I won't guess. "
...
def answer_question(query: str, index: RagIndex, top_k: int = 4,
    ...
    for ch, score in hits:
        ...
        # Require genuine term overlap, OR a strong standalone TF-IDF score that the
        # company-name boost alone cannot reach (the boost adds ~0.15, so a 0.30
        # floor here ensures a real lexical match carried the result, not the boost).
        if content_overlap >= 1 or score >= 0.30:
